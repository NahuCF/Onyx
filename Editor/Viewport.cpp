...
void Viewport::Loop(ImGuiIO& io)
{
	...
	if (m_Window->IsButtomPressed(GLFW_MOUSE_BUTTON_LEFT) && map->GetCurrentTile().x >= 0 && map->GetCurrentTile().y >= 0 && map->GetCurrentTile().x < map->GetTilemapSize().x && map->GetCurrentTile().y < map->GetTilemapSize().y && map->GetTileValue() == 0)
	{
