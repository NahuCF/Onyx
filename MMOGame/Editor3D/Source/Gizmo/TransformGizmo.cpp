...
namespace MMO {
									  ...
									  const glm::vec3& gizmoPos, float cameraDistance)
	{
		...
		if (m_Mode == GizmoMode::TRANSLATE || m_Mode == GizmoMode::SCALE)
		{
			...
			if (xHit && xDist < minDist)
			{
				minDist = xDist;
				closest = GizmoAxis::X;
			}
			if (yHit && yDist < minDist)
			{
				minDist = yDist;
				closest = GizmoAxis::Y;
			}
			if (zHit && zDist < minDist)
			{
				minDist = zDist;
				closest = GizmoAxis::Z;
			}
								   ...
								   const glm::vec3& rayDir, const glm::mat4& view)
	{
		...
		{
		case GizmoAxis::X:
			m_DragAxisDir = glm::vec3(1, 0, 0);
			break;
		case GizmoAxis::Y:
			m_DragAxisDir = glm::vec3(0, 1, 0);
			break;
		case GizmoAxis::Z:
			m_DragAxisDir = glm::vec3(0, 0, 1);
			break;
		default:
			m_DragAxisDir = glm::vec3(0, 0, 0);
			break;
		}
