...
bool Subprocess::IsRunning() {
    if (!m_Started || m_Exited) return false;
...
bool Subprocess::IsRunning() {
    if (m_Pid <= 0 || m_Exited) return false;
    int status = 0;
    ...
    if (r == m_Pid) {
        ...
        else if (WIFSIGNALED(status)) m_ExitCode = 128 + WTERMSIG(status);
        m_Pid = -1;
        return false;
    }
    if (errno == ECHILD) {
