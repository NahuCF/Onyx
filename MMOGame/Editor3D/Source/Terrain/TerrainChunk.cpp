...
namespace Editor3D {
	...
	int TerrainChunk::FindMaterialLayer(const std::string& materialId) const
	{
		for (int i = 0; i < MAX_TERRAIN_LAYERS; i++)
		{
			if (m_Data.materialIds[i] == materialId)
				return i;
		}
		return -1;
	}
	...
	{
		if (!m_VAO || m_State != ChunkState::Active)
			return;
	...
	void TerrainChunk::GenerateMesh()
	{
		...
		if (!m_VAO)
		{
			...
			VertexLayout layout({{VertexAttributeType::Float3},
