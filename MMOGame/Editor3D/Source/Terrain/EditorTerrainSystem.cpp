...
void EditorTerrainSystem::Init(const std::string& projectPath) {
    m_ProjectPath = projectPath.empty() ? "." : projectPath;

    std::filesystem::create_directories(m_ProjectPath + "/terrain");

    std::string terrainDir = m_ProjectPath + "/terrain";
    if (std::filesystem::exists(terrainDir)) {
...
std::string EditorTerrainSystem::GetChunkFilePath(int32_t chunkX, int32_t chunkZ) const {
    return m_ProjectPath + "/terrain/chunk_" + std::to_string(chunkX) + "_" + std::to_string(chunkZ) + ".terrain";
}
...
void EditorTerrainSystem::SaveChunkToDisk(TerrainChunk* chunk) {
    ...
    std::string dir = m_ProjectPath + "/terrain";
