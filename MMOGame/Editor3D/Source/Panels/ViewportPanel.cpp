...
namespace MMO {
	...
	void ViewportPanel::RenderScene()
	{
		...
		m_SceneRenderer->RenderShadows([this](uint32_t cascade, const glm::mat4& lightSpaceMat) {
			...
			if (m_World)
			{
				for (const auto& obj : m_World->GetStaticObjects())
				{
					if (!obj->IsVisible() || !obj->CastsShadow())
						continue;
			...
			if (m_TerrainEnabled)
			{
				...
				for (auto& [key, wchunk] : m_WorldSystem.GetChunks())
				{
					...
					if (terrain->GetState() != Editor3D::ChunkState::Active)
						continue;
					if (!cascadeFrustum.IsBoxVisible(terrain->GetBoundsMin(), terrain->GetBoundsMax()))
						continue;
	...
	void ViewportPanel::RenderWorldObjects()
	{
...

		{
			...
			for (const auto& obj : m_World->GetStaticObjects())
			{
				if (!obj->IsSelected() || !obj->IsVisible())
					continue;
				const std::string& modelPath = obj->GetModelPath();
				if (modelPath.empty() || modelPath[0] == '#')
					continue;
	...
	{
		if (!m_World || !m_Gizmo || !m_ViewportHovered)
			return;
		...
		if (m_Gizmo->IsDragging() && ImGui::IsMouseDown(ImGuiMouseButton_Left))
		{
			...
			if (isMeshSelected && meshMaterial && staticObj)
			{
				...
				else if (mode == GizmoMode::ROTATE)
				{
					...
					if (axis == GizmoAxis::X)
						newRotation.x += deltaDegrees;
					else if (axis == GizmoAxis::Y)
						newRotation.y += deltaDegrees;
					else if (axis == GizmoAxis::Z)
						newRotation.z += deltaDegrees;
	...
	{
		if (!m_World || !m_ViewportHovered)
			return;

		if (m_Gizmo && m_Gizmo->GetActiveAxis() != GizmoAxis::NONE)
			return;
	...
	{
		if (!m_World || !m_World->HasSelection())
			return;
	...
	ViewportPanel::ResolvedModel& ViewportPanel::ResolveModelCached(const std::string& path, bool checkAnimated)
	{
		...
		if (it != m_ResolvedModelCache.end())
			return it->second;
		...
		else if (status == Onyx::ModelLoadStatus::Ready)
		{
			...
			if (resolveMs > m_ResolveModelTime)
				m_ResolveModelTime = resolveMs;
	...
	{
		if (!m_TerrainShader || !m_SceneRenderer)
			return;
		...
		if (lib.GetDiffuseArray())
			lib.GetDiffuseArray()->Bind(3);
		if (lib.GetNormalArray())
			lib.GetNormalArray()->Bind(4);
		if (lib.GetRMAArray())
			lib.GetRMAArray()->Bind(5);
	...
	void ViewportPanel::HandleTerrainInput()
	{
		...
		if (m_TerrainTool.mode == TerrainToolMode::Ramp)
		{
			...
			if (m_TerrainTool.brushValid && ImGui::IsMouseClicked(ImGuiMouseButton_Left) && !m_RightMouseDown && !m_MiddleMouseDown)
			{
				...
				else
				{
					...
					m_WorldSystem.BeginEdit(
						...
						glm::length(glm::vec2(m_TerrainTool.brushPos.x - m_TerrainTool.rampStart.x,
											  m_TerrainTool.brushPos.z - m_TerrainTool.rampStart.z)) +
							rampWidth);
	...
	void ViewportPanel::CollectLights()
	{
		...
		auto byDist = [&](const Editor3D::EditorLight& a, const Editor3D::EditorLight& b) {
			return glm::distance(a.position, m_CameraPosition) < glm::distance(b.position, m_CameraPosition);
		};
