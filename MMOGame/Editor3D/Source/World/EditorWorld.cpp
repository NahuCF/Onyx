...
namespace MMO {
	...
	void EditorWorld::ForEachObject(const std::function<void(WorldObject*)>& callback)
	{
		for (auto& obj : m_StaticObjects)
			callback(obj.get());
		for (auto& obj : m_SpawnPoints)
			callback(obj.get());
		for (auto& obj : m_Lights)
			callback(obj.get());
		for (auto& obj : m_ParticleEmitters)
			callback(obj.get());
		for (auto& obj : m_TriggerVolumes)
			callback(obj.get());
		for (auto& obj : m_InstancePortals)
			callback(obj.get());
		for (auto& obj : m_PlayerSpawns)
			callback(obj.get());
	...
	void EditorWorld::ForEachObject(const std::function<void(const WorldObject*)>& callback) const
	{
		for (const auto& obj : m_StaticObjects)
			callback(obj.get());
		for (const auto& obj : m_SpawnPoints)
			callback(obj.get());
		for (const auto& obj : m_Lights)
			callback(obj.get());
		for (const auto& obj : m_ParticleEmitters)
			callback(obj.get());
		for (const auto& obj : m_TriggerVolumes)
			callback(obj.get());
		for (const auto& obj : m_InstancePortals)
			callback(obj.get());
		for (const auto& obj : m_PlayerSpawns)
			callback(obj.get());
		for (const auto& obj : m_Groups)
			callback(obj.get());
	}
	...
	{
		if (!m_HasMeshClipboard || !HasMeshSelected() || m_SelectedMeshName.empty())
			return;
