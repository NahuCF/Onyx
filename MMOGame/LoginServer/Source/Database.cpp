...
std::vector<CharacterData> Database::GetCharactersByAccountId(AccountId accountId) {
    ...
    try {
        ...
        for (const auto& row : result) {
            ...
            c.zoneId = row[6].as<std::string>();
            c.positionX = row[7].as<float>();
            c.positionY = row[8].as<float>();
            c.maxHealth = row[9].as<int32_t>();
            c.maxMana = row[10].as<int32_t>();
            c.currentHealth = row[11].as<int32_t>();
            c.currentMana = row[12].as<int32_t>();
            c.lastPlayed = row[13].as<uint64_t>();
...
bool Database::CreateCharacter(AccountId accountId, const std::string& name,
    ...
    try {
        ...
        pqxx::work txn(*m_Connection);
        pqxx::result result = txn.exec_params(
            "INSERT INTO characters (account_id, name, class, max_health, max_mana, current_health, current_mana) "
...
bool Database::SaveCharacter(const CharacterData& character) {
    try {
        ...
        txn.exec_params(
            ...
            "level = $2, experience = $3, zone_id = $4, position_x = $5, position_y = $6, "
            "max_health = $7, max_mana = $8, current_health = $9, current_mana = $10, "
            "last_played = CURRENT_TIMESTAMP "
            "WHERE id = $1",
            character.id, character.level, character.experience, character.zoneId,
