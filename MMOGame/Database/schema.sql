-- MMO Game Database Schema
-- PostgreSQL
--
-- Development usage: DROP and recreate the database, then run this file.
...
CREATE TABLE IF NOT EXISTS characters (
    ...
    race            SMALLINT NOT NULL DEFAULT 1,  -- 1=Human, 2=Orc
    class           SMALLINT NOT NULL,            -- 1=Warrior, 2=Witch
    level           INT DEFAULT 1,
    experience      BIGINT DEFAULT 0,
    money           INT DEFAULT 0,                -- Total copper (1g=10000c, 1s=100c)
    ...
    -- Meta
    created_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    played_time     BIGINT DEFAULT 0,  -- seconds
...
CREATE TABLE IF NOT EXISTS character_cooldowns (
    ...
    remaining       REAL NOT NULL,  -- Seconds remaining
...
CREATE TABLE IF NOT EXISTS character_inventory (
    ...
    character_id    BIGINT NOT NULL REFERENCES characters(id) ON DELETE CASCADE,
    slot            SMALLINT NOT NULL,  -- 0-19 for 20-slot inventory
...
CREATE TABLE IF NOT EXISTS character_equipment (
    instance_id     BIGINT PRIMARY KEY DEFAULT nextval('item_instance_seq'),
    character_id    BIGINT NOT NULL REFERENCES characters(id) ON DELETE CASCADE,
    slot            SMALLINT NOT NULL,  -- 0-13 for equipment slots (HEAD=0 through OFFHAND=13)
    template_id     INT NOT NULL,
    UNIQUE(character_id, slot)
);
...
-- ============================================================
-- MAP TEMPLATE (replaces hardcoded MapTemplates.cpp)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS race_template (
    ...
    class_mask      INT NOT NULL DEFAULT 0,        -- bitmask: bit0=Warrior, bit1=Witch
...
-- ============================================================
-- PLAYER CREATE INFO (per race+class spawn location)
-- ============================================================
...
BEGIN
    CASE class_id
        WHEN 1 THEN  -- Warrior: Rusty Sword + Wooden Shield
            INSERT INTO character_equipment (character_id, slot, template_id)
            VALUES (char_id, 12, 1001);  -- WEAPON
            INSERT INTO character_equipment (character_id, slot, template_id)
            VALUES (char_id, 13, 1020);  -- OFFHAND
        WHEN 2 THEN  -- Witch: Staff of Flames
            INSERT INTO character_equipment (character_id, slot, template_id)
            VALUES (char_id, 12, 1010);  -- WEAPON
        ELSE
            INSERT INTO character_equipment (character_id, slot, template_id)
            VALUES (char_id, 12, 1001);
    END CASE;

    INSERT INTO character_inventory (character_id, slot, template_id)
    VALUES
        (char_id, 0, 2001),  -- Cloth Hood
        (char_id, 1, 2101),  -- Cloth Robe
        (char_id, 2, 2201),  -- Cloth Pants
        (char_id, 3, 2301);  -- Cloth Shoes
END;
$$ LANGUAGE plpgsql;
...
-- Player create info (race, class, map_id, x, y, z, orientation)
INSERT INTO player_create_info VALUES (1, 1, 1, 10.0, 10.0, 0.0, 0.0)
    ON CONFLICT (race, class) DO NOTHING;
INSERT INTO player_create_info VALUES (1, 2, 1, 10.0, 10.0, 0.0, 0.0)
    ON CONFLICT (race, class) DO NOTHING;
INSERT INTO player_create_info VALUES (2, 1, 1, 10.0, 10.0, 0.0, 0.0)
    ON CONFLICT (race, class) DO NOTHING;
INSERT INTO player_create_info VALUES (2, 2, 1, 10.0, 10.0, 0.0, 0.0)
    ON CONFLICT (race, class) DO NOTHING;
