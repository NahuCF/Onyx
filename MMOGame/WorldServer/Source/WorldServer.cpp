...
namespace MMO {
	...
	WorldServer::WorldServer()
		: m_Running(false), m_ServerTick(0)
	{
	}
	...
	void WorldServer::HandleAuthToken(uint32_t peerId, ReadBuffer& buf)
	{
		...
		if (mapTemplateId == 0)
			mapTemplateId = 1; // Default to starting zone
	...
	void WorldServer::SendWorldState(MapInstance* map)
	{
		...
		for (EntityId entityId : dirtyEntities)
		{
			Entity* entity = map->GetEntity(entityId);
			if (!entity)
				continue; // Entity may have been despawned
			...
			if (flags.position)
				updateMask |= UPDATE_POSITION;
			if (flags.health)
				updateMask |= UPDATE_HEALTH;
			if (flags.mana)
				updateMask |= UPDATE_MANA;
			if (flags.target)
				updateMask |= UPDATE_TARGET;
			if (flags.casting)
				updateMask |= UPDATE_CASTING;
			if (flags.moveState)
				updateMask |= UPDATE_MOVE_STATE;
			...
			if (combat)
			{
				...
				update.castProgress = combat->castDuration > 0 ? (combat->castTimer / combat->castDuration) : 0.0f;
			}
	...
	void WorldServer::SendSpawnsAndDespawns(MapInstance* map)
	{
		...
		for (const auto& [playerEntityId, info] : map->GetAllPlayers())
		{
			...
			if (!playerEntity || !playerEntity->GetMovement())
				continue;
	...
	void WorldServer::HandleEquipItem(uint32_t peerId, ReadBuffer& buf)
	{
		...
		Entity* entity = map->GetEntity(player.entityId);
		if (!entity || !entity->GetInventory() || !entity->GetEquipment())
			return;
	...
	{
		if (!player || !player->GetInventory() || !m_Database.IsConnected())
			return;
		...
		{
			if (itemData.slot >= INVENTORY_SIZE)
				continue;
	...
	{
		if (!player || !player->GetEquipment() || !m_Database.IsConnected())
			return;
		...
		for (const auto& itemData : items)
		{
			if (itemData.slot >= EQUIPMENT_SLOT_COUNT)
				continue;
